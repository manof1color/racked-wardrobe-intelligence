...
- Multi-piece foreground isolation is display preparation, not virtual try-on or body-fit prediction. Bedrock segments existing garment pixels into a transparent PNG; the server validates and trims the result. AI segmentation, silhouette isolation, and conservative edge transparency are independent optional enhancements, and the final fallback keeps the ordinary bounded photo. A recognition-provider failure creates a zero-confidence editable manual-review item rather than inventing attributes or blocking intake. Existing garments are not silently rewritten—new whole-look scans receive this processing.
