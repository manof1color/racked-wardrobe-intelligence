...
The gate is `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `pnpm audit:prod`. The current implementation carries **200 passing tests** (re-verified 2026-08-21). Consumer Hanger recognizes natural revision wording, retains up to 100 owner-validated prior recommendation IDs, rotates repeated outfit-creation prompts through unseen pieces, and uses one canonical ordered selection for the written list, owned-piece image projection, Save/Wear IDs, saved title, and private board. Model prose and client actions are checked against that selection before persistence. Consumers can remove individual saved-outfit pieces with a regenerated private board. AI cannot grant verified brand identity. Brand and Community responses exclude private wardrobe records, owner IDs, emails, evidence images, S3 keys, and individual wear histories. Private wear remains consent-filtered and suppressed below 25 owners. Public Community metrics use only published records and identity-free events.
