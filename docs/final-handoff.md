...
- Multi-piece foreground isolation is display preparation, not virtual try-on or body-fit prediction. Synchronous intake uses local silhouette isolation, conservative edge transparency, and finally the ordinary bounded photo, so one scan never creates a remote cleanup call for every detected piece. The validated Bedrock segmentation helper remains available for a future asynchronous flow but is not an intake gate. A recognition-provider failure creates a zero-confidence editable manual-review item rather than inventing attributes or blocking intake. Existing garments are not silently rewritten—new whole-look scans receive this processing.
