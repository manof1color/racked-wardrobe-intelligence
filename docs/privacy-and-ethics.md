...
- Authenticated app links preserve the signed-in role workspace and never clear a session as a navigation side effect. The logo, Community, and mobile tabs remain inside the authenticated shell; only the explicit Sign out action calls the logout endpoint.
