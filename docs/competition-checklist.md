...
- [x] Automatic rotation plus a request-budget-safe garment display pipeline — silhouette isolation, conservative edge removal, then the ordinary bounded photo — with private original evidence preserved. Synchronous intake makes no per-piece provider call after recognition. Each optional local step can fail independently without blocking intake. `scripts/crop-benchmark.ts` scores the deterministic passes by intersection-over-union against a known garment rectangle across 14 seeded backdrops: the border trim baseline reached 61% mean IoU (7/14 usable), and the silhouette pass reaches 85% (12/14). Its largest gains are the cases that previously produced a visibly wrong crop — a second object in shot (44% to 97%) and neighbours on a crowded rail (37% to 97%).
...
- [x] 347 automated tests and lint pass (re-verified 2026-09-14); the clean CI gate runs type check, tests, production build, production dependency audit, and CodeQL before merge.
