...
- [x] Session behavior is regression-tested for valid round-trips, token tampering and malformed input, exact expiry, password-change session-version invalidation, deleted accounts, and Consumer/Brand route separation; the exercised account-state helper is the same one used by the live route path.
...
- [x] Up to six photos can be scanned in one batch: each is recognised on its own, every piece lands in one review list tagged with its photo, an unreadable photo never discards the pieces already found, and the list is bounded at 24 with an honest summary.
...
- [x] Automatic rotation plus a request-budget-safe garment display: every detected piece is stored as its bounded crop with private original evidence preserved, and synchronous intake makes no per-piece provider call after recognition. Background removal is off in live intake after it erased white garments on real phone photos; the local isolation passes remain measured research. `scripts/crop-benchmark.ts` scores the deterministic passes by intersection-over-union against a known garment rectangle across 14 seeded backdrops: the border trim baseline reached 61% mean IoU (7/14 usable), and the silhouette pass reaches 85% (12/14). Its largest gains are the cases that previously produced a visibly wrong crop — a second object in shot (44% to 97%) and neighbours on a crowded rail (37% to 97%).
...
- [x] Garment intake is one flow rather than two modes. A person no longer has to decide, before photographing anything, whether a garment is an enrolled brand product; every piece arrives the same way and brand linking is an optional per-piece upgrade that checks label evidence against the registry. This closed a real gap — only the three-photo path ever consulted the registry, so a genuine brand product added the quick way could never be linked. Each piece shows plainly whether it is a brand product, an ordinary wardrobe piece, or one AI could not classify.
- [x] When recognition is unsure, the intake **Type** field asks rather than guesses. It accepts typed text with suggestions, maps known words onto the controlled taxonomy, keeps unmatched words in the person's own phrasing beside the category's *Other* type, and shows its note beneath the field so the photograph is never covered — regression-tested against any overlay positioning. Typed words never verify a brand.
- [x] Scan and Closet previews always show the whole recognised piece: crops keep an 8% margin and are displayed contained, never zoomed to fill the tile or covered by labels. On iPhone the bottom tab bar and Hanger button are held on the visible bottom edge after the keyboard closes and step aside while typing (unit-tested; physical-iPhone recheck pending).
- [x] The landing page is one responsive grid with nothing positioned over its copy, a 0.72rem type floor, numbering only on the genuine five-step flow, and example figures visibly labelled as examples. Motion is progressive — a staggered rise on load, scroll-driven section reveals where supported, hover lifts — and fully disabled under reduced-motion preferences.
- [x] Owner-scoped deletion. A garment can be deleted with outfits, the owner's Community posts, brand wear events, and photos kept consistent, and the record removed last so retries finish. A consumer account can be deleted in-app with the current password and a typed confirmation, as Apple Guideline 5.1.1(v) and Google Play require. Brand account deletion is scheduled, not claimed.
- [x] The verification boundary is unchanged and regression-tested at the new entry point: a brand name alone verifies nothing, a match requires a GTIN or brand-plus-SKU registry evidence, and the label-check route writes nothing. Identical image files and file names are no longer evidence anywhere, codes match only as whole codes, and a successful label check is re-matched on the server at save so the verified link actually persists.
- [x] Brand products can be linked without the care label: a scanned piece is ranked against the enrolled catalog (up to three look-alikes with reasons), or the person searches by brand, name, or style code. The result is saved as the owner's **pick** — shown to them with cost per wear from the listed price — and is regression-tested never to become verified identity, enter the brand owner index, or make a Community piece shoppable.
- [x] Garment display output is validated before it reaches anyone, not trusted because a pass reported success. A detection crop must retain the garment as at least 28% of the frame, and one connected region must account for at least 95% of what survives background removal — a garment being one object. Both thresholds are measured, and when every pass fails them the person gets the ordinary bounded photograph rather than a shredded cutout. Live intake now goes further and never shows a cut-out at all.
...
- [x] 564 automated tests and lint pass (re-verified 2026-09-22); the clean CI gate runs type check, tests, production build, production dependency audit, and CodeQL before merge.
- [x] Synthetic demo images are generated from one offline, deterministic illustrator shared by both seeds: distinct category silhouettes, subtype-specific pants/shoes/accessories, visible `SYNTHETIC DEMO` marking, no embedded photographs, and a raster-size regression test. Existing hosted images require a seed rerun to receive the new pixels; no product or wear records change.
