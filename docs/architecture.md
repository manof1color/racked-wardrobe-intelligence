...
In production, provider failure at classification, whole-look recognition, or multi-view analysis degrades to a documented deterministic path: the standard back-plus-label photo set or an explicitly zero-confidence unverified manual-review result. Whole-look display preparation treats silhouette isolation and edge removal as independent enhancements; if both are unavailable, the bounded original crop remains usable. The optional AI background-removal helper is retained for future asynchronous processing but is not called by synchronous intake. The Consumer can save their own reviewed labels, but Racked creates no invented AI attributes or verified product link.
