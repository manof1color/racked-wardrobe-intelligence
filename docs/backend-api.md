...
| `PATCH /api/consumer/wardrobe` | Consumer | Edits one owned garment's name, category, type, colour, pattern, material, style, season, or brand. The owner is the session, and the update is conditional on the record already existing, so an edit can neither reach another account's piece nor create one. Unknown keys are dropped, and a fixed field allowlist in the store keeps wear history, photos, and registry identity out of reach. A catalog product chosen here is the person's own pick, never verified and never joined to a brand's owner index; a piece verified from its care label keeps its brand details (409) while everything else remains editable — including a piece verified before verification status was recorded, which is recognised by its registry link and owner-index entry rather than by the status field alone. Rate-limited separately from deletion |
...
| `GET /api/version` | Public | Returns the commit the running artifact was built from and the time that build ran, uncached. A server-side change leaves every content-hashed asset identical and every gated route answering the same 401, so this is the only way to confirm from outside that a deployment actually carries a change. Reads no account, configuration, credential, or request data; the commit SHA of a public repository is already public |
