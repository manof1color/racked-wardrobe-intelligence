...
| `POST /api/garments/detect` | Consumer | Accepts one prepared general photo, asks Bedrock for up to eight bounded distinct wearable instances, rejects unknown/tiny/duplicate detections, stores the source and per-piece crops privately, and returns HMAC-signed editable candidates; visible brand text is suggestion-only and cannot verify identity |
