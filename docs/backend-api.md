...
| `POST /api/garments/detect` | Consumer | Accepts one prepared general photo, asks Bedrock for up to eight bounded distinct wearable instances, rejects unknown/tiny/duplicate detections, stores the source and per-piece display images privately, and returns HMAC-signed editable candidates. A conservative deterministic pass marks a consistent edge-connected background transparent and otherwise keeps an opaque crop; `processedImage.backgroundRemoved` reports which result occurred. Visible brand text is suggestion-only and cannot verify identity |
