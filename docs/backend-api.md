...
| `POST /api/garments/detect` | Consumer | Accepts one prepared general photo, asks Bedrock for up to 16 bounded wardrobe units after a full-image coverage pass, and treats matching left/right footwear as one `pair`. A shared provider pair ID deterministically combines split sides without merging adjacent different pairs. It rejects unknown/tiny/duplicate detections, stores the source and per-unit display images privately, and returns HMAC-signed editable candidates. A US-only Stable Image task isolates each crop into a validated, trimmed transparent PNG without generating garment pixels; provider failure falls back to conservative edge transparency or an honest opaque tight crop. `processedImage.backgroundRemoved` and `backgroundRemovalMethod` report the result. Visible brand text is suggestion-only and cannot verify identity |
