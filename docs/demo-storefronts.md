...
> Every page under `/demo-store` is a **fictional shop for a fictional brand**. Nothing is for sale, there is no cart or checkout, no payment is processed, and no personal detail is collected. Each page carries a persistent banner saying so.
...
- The "Add to bag" control is permanently `disabled` with `aria-disabled`, and the page states in text that it is non-functional.
