...
Outfits are also kept seasonally coherent: a summer piece and a winter piece are not placed in one outfit while a coherent piece of the same category exists. Only that direct contradiction counts — all-season, spring, fall, and untagged pieces pair with anything — a clash is accepted rather than leaving a core slot empty, an optional extra that clashes is simply left out, and pieces the person names are never dropped for a season. Each piece had previously been scored alone, so without a weather cue the judge closet produced a winter knit over summer linen shorts. The Looks screen opens on this ranker's answer to "What should I wear today?", and its Shuffle and Finish controls call the same ranker around the person's locked rows.
