...
The default Consumer Add mode sends one user-selected photo to Amazon Nova Lite and requests up to 16 distinct visible wardrobe units with normalized bounding coordinates, controlled category/subtype, visible attributes, confidence, and evidence. The prompt requires a systematic full-image scan plus a second coverage check across shelves, edges, and partly obscured regions. Matching left/right shoes are one `pair` unit with one crop; if the provider emits separate sides, a shared pair identifier lets deterministic application code combine their bounds. The parser never merges footwear merely for looking similar, so adjacent different pairs remain independent. Application code also rejects unknown, tiny, and near-duplicate detections before Sharp creates a bounded private crop for each candidate.

Each crop is then sent to the Stable Image Remove Background model through its US geographic inference profile (`us.stability.stable-image-remove-background-v1:0`). The operation isolates existing foreground pixels; it is not text-to-image generation and does not redesign the garment. Racked accepts only a PNG with a real alpha channel and a non-degenerate transparent/foreground balance, trims transparent margins, adds neutral padding, and records `backgroundRemovalMethod="ai-segmentation"`. Invalid output or provider failure returns control to the deterministic edge-connected flood fill; that fallback either makes a safe plain-background cutout or retains the opaque crop. The Consumer sees the result, chooses which candidates to keep, and can correct every label before saving.
