...
- Amplify compute role scoped to required DynamoDB, private S3-object, and Amazon Bedrock actions. The template also grants `s3:ListBucket` limited to the `wardrobe/` prefix, so account deletion can remove scan photos that were never saved. **That permission takes effect only after the stack is updated**; until then deletion completes without the sweep and logs the gap.
...
Amplify is configured with the table and bucket names, the compute role, `AI_PROVIDER=bedrock`, and `AI_MODEL=amazon.nova-lite-v1:0`. Routine garment analysis and Hanger therefore stay on Nova Lite. Whole-look instance detection defaults in code to the US Nova Pro geographic inference profile (`us.amazon.nova-pro-v1:0`), and `AI_LOOK_DETECTION_MODEL` can override only that task. The compute role permits the Nova foundation-model and account inference-profile resources required by this split. Synchronous whole-look intake uses local crop isolation rather than invoking the configured Stable Image helper per piece. Secret values are not printed or stored in GitHub.
...
# Whole-look detection only. Omission uses the same code default; setting it explicitly
# makes the deployed model policy visible in Amplify without changing routine AI calls.
AI_LOOK_DETECTION_MODEL=us.amazon.nova-pro-v1:0
...
- [x] Amazon Nova Lite is available for routine calls from `us-east-2`; the US Nova Pro geographic profile accepts `us-east-2` as a source and routes only among documented US destination regions. Stable Image background removal is retained as a future asynchronous option rather than a synchronous per-piece intake dependency.
- [x] Direct Bedrock response test passed.
- [x] Local lint passed.
- [x] All 365 automated tests pass (re-verified 2026-09-16); clean CI type-check, build, production dependency audit, and CodeQL remain mandatory before merge.
