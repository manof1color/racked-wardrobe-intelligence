...
- **Consumer Hanger:** a multi-turn agent reloads only the signed-in consumer's wardrobe, wear history, and saved outfits, then returns grounded styling guidance and validated save/wear actions. One canonical server selection drives the written garment list, private photo cards, action IDs, saved title, and flat-lay order; generated prose that names a different owned garment is rejected.
...
The suite currently has **200 passing tests** (verified 2026-08-21), covering privacy suppression and the enumeration budget, the registry-only verification boundary, deterministic Recreate and outfit-ranking scoring, four-turn Hanger conversation memory, canonical name/image/save alignment, owner-scoped saved-outfit and piece management, commerce URL validation, Brand Look ownership, account recovery, and public-field sanitization.
...
| Code, docs & GitHub | 15% | Typed modules, **200 passing tests**, CI running audit + lint + typecheck + tests + build, CodeQL, and incremental reviewed PRs ([PROGRESS.md](PROGRESS.md)) |
