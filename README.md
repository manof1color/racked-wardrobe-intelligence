...
2. Enroll authorized front, back, and label images with SKU/MPN, optional GTIN, aliases, and approved label text. A GTIN must pass its GS1 check digit; one GTIN, and one style code per brand, maps to exactly one product; and an alias may not name another brand. Every check runs before any photo is stored.
...
| Code, docs & GitHub | 15% | Typed modules, **426 passing tests**, CI running audit + lint + typecheck + tests + build, CodeQL, and incremental reviewed PRs ([PROGRESS.md](PROGRESS.md)) |
