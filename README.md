...
**Infrastructure:** AWS Amplify Hosting (SSR) · DynamoDB single-table, on-demand · private encrypted S3 with public access blocked · Amazon Bedrock in `us-east-2`. The Amplify compute role has only the DynamoDB, S3-object, Bedrock, and SES permissions these flows require (`infra/template.yaml`). No AWS credentials or secrets are committed to GitHub.
...
- **Server-side outfit ranking:** which garments Hanger proposes is decided by weighted scoring on the server — occasion, weather, requested style, underuse, and time since last worn — never by the model, which cannot introduce or invent an item. Selection is deterministic, every piece carries inspectable score components, and a follow-up sets aside what was already suggested so asking for something else returns something else.
...
8. Hanger opens from the bottom of the dashboard as a multi-turn stylist. Every message reloads the account's current wardrobe, wear history, and saved outfits; grounded recommendations can be saved or recorded as worn, and a successful Hanger save appears in the Outfits tab immediately without a reload. When the consumer asks for a different look, validated prior recommendation IDs and the latest saved outfit are used to maximize new pieces; repeats occur only when the wardrobe cannot fill every slot differently. Hanger lists the exact pieces attached to the Save action, and the saved outfit name reflects those pieces.
...
| `GET/POST /api/consumer/wardrobe` · `/outfits` · `/consent` | Consumer | Always scoped to the signed-in account |
...
The suite currently has **187 passing tests** (verified 2026-08-21), covering privacy suppression and the enumeration budget, the registry-only verification boundary, deterministic Recreate and outfit-ranking scoring, distinct Hanger recommendations, saved-outfit management, commerce URL validation, Brand Look ownership, account recovery, and public-field sanitization.
...
| Code, docs & GitHub | 15% | Typed modules, **187 passing tests**, CI running audit + lint + typecheck + tests + build, CodeQL, and incremental reviewed PRs ([PROGRESS.md](PROGRESS.md)) |
...
**Install on a phone:** open the [HTTPS application](https://main.d2iv0khybuuaeh.amplifyapp.com). On iPhone use **Safari → Share → Add to Home Screen**; on Android use **Chrome → Install app**.
