...
The suite currently has **196 passing tests** (verified 2026-08-21), covering privacy suppression and the enumeration budget, the registry-only verification boundary, deterministic Recreate and outfit-ranking scoring, four-turn Hanger conversation memory and exact image previews, owner-scoped saved-outfit and piece management, commerce URL validation, Brand Look ownership, account recovery, and public-field sanitization.
...
| Code, docs & GitHub | 15% | Typed modules, **196 passing tests**, CI running audit + lint + typecheck + tests + build, CodeQL, and incremental reviewed PRs ([PROGRESS.md](PROGRESS.md)) |
