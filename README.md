...
8. Hanger opens from the bottom of the dashboard as a multi-turn stylist. Every message reloads the account’s current wardrobe, wear history, and saved outfits; grounded recommendations can be saved or recorded as worn. A successful Hanger save appears in the Outfits tab immediately without a reload. When the consumer asks for a different look, validated prior recommendation IDs and the latest saved outfit are used to maximize new pieces; repeats occur only when the wardrobe cannot fill every slot differently. Hanger lists the exact pieces attached to the Save action, and the saved outfit name reflects those pieces.
...
| Code, docs & GitHub — 15% | Typed modules, **187 passing tests** incl. account recovery, own-account authorization, distinct Hanger recommendations, saved-outfit management, right-brand matching, flat-lay layout, multi-piece detection, commerce, ownership, and privacy suites; CI runs lint + typecheck + tests + build + audit, CodeQL, and incremental PRs ([PROGRESS.md](PROGRESS.md)) |
