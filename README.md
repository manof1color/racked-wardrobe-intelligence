...
8. Hanger opens from the bottom of the dashboard as a multi-turn stylist. Every message reloads the account’s current wardrobe, wear history, and saved outfits; grounded recommendations can be saved or recorded as worn. A successful Hanger save appears in the Outfits tab immediately without a reload.
...
| Code, docs & GitHub — 15% | Typed modules, **182 passing tests** incl. account recovery, own-account authorization, saved-outfit deletion/Hanger save handoff, right-brand matching, flat-lay layout, multi-piece detection, commerce, ownership, and privacy suites; CI runs lint + typecheck + tests + build + audit, CodeQL, and incremental PRs ([PROGRESS.md](PROGRESS.md)) |
