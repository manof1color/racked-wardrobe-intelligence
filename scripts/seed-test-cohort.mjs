...
const createdAt="2026-08-13T12:00:00.000Z",wearPattern=[0,1,2,2,3,3,4,5,6,8],publicOrigin=(process.env.RACKED_PUBLIC_ORIGIN??"https://main.d2iv0khybuuaeh.amplifyapp.com").replace(/\/+$/,"");
const cohorts=[
  ...
  {key:"stride",brand:"Synthetic Stride Lab",slug:"synthetic-stride-lab",email:"demo.footwear@racked.local",accent:"#ff684b",base:"#292929",prefix:"SSL",categories:Array(10).fill("shoe"),names:["City Sneaker","Track Runner","Archive Loafer","Trail Boot","Court Low","Formal Derby","Summer Sandal","Studio Trainer","Chelsea Boot","Canvas Slip-On"]},
...
for(const cohort of cohorts){
  ...
  for(let index=0;index<10;index++){
    ...
    const product={id:productId,ownerSubject:brandId,name:cohort.names[index],brand:cohort.brand,brandSlug:cohort.slug,aliases:[cohort.brand,cohort.prefix],sku,gtin:null,category:cohort.categories[index],subtype:cohort.categories[index]==="shoe"?"sneakers":cohort.categories[index]==="jewelry"?"necklace":cohort.categories[index]==="top"?"t-shirt":cohort.categories[index]==="bottom"?"casual-pants":cohort.categories[index]==="outerwear"?"bomber-jacket":"accessory",color:cohort.base,pattern:"solid",material:"synthetic demo material",style:["casual","synthetic demo"],labelText:`${cohort.brand} ${sku} SYNTHETIC DEMO PRODUCT`,views,enrolledAt:createdAt,source:"brand-enrolled",productUrl:`${publicOrigin}/demo-store/${encodeURIComponent(cohort.slug)}/${encodeURIComponent(sku)}`,price:45+(cohorts.indexOf(cohort)*35)+(index*7),currency:"USD",availability:cohort.key==="lumen"&&index===9?"unavailable":"available",testCohort:true,dataClassification:"DEMO",PK:`USER#${brandId}`,SK:`PRODUCT#${productId}`,GSI1PK:"BRAND_PRODUCTS",GSI1SK:`${cohort.slug}#${sku}`};
  ...
  for(let lookIndex=0;lookIndex<2;lookIndex++){
    ...
    const postId=`demo-post-${cohort.key}-brand-${lookIndex+1}`,publishedGarments=lookProducts.map((product,itemIndex)=>({publicGarmentId:`${postId}-item-${itemIndex+1}`,name:product.name,category:product.category,subtype:product.subtype,color:product.color,pattern:product.pattern,material:product.material,style:product.style,imageKey:product.views.front.storageKey,resolutionState:"EXACT_VERIFIED_PRODUCT",verifiedProduct:{registryProductId:product.id,sku:product.sku,name:product.name,brand:product.brand,brandSlug:product.brandSlug,commerceState:"EXACT_AVAILABLE",outboundUrl:`/api/products/${encodeURIComponent(product.id)}/outbound?sourcePostId=${encodeURIComponent(postId)}`,price:product.price,currency:product.currency}}));
...
for(let index=1;index<=25;index++){
  ...
  if(index<=10){const postId=`demo-consumer-post-${suffix}`,publishedGarments=outfitItems.map((item,itemIndex)=>({publicGarmentId:`${postId}-item-${itemIndex+1}`,name:item.product.name,category:item.product.category,subtype:item.product.subtype,color:item.product.color,pattern:item.product.pattern,material:item.product.material,style:item.product.style,imageKey:item.imageKey,resolutionState:"EXACT_VERIFIED_PRODUCT",verifiedProduct:{registryProductId:item.product.id,sku:item.product.sku,name:item.product.name,brand:item.product.brand,brandSlug:item.product.brandSlug,commerceState:"EXACT_AVAILABLE",outboundUrl:`/api/products/${encodeURIComponent(item.product.id)}/outbound?sourcePostId=${encodeURIComponent(postId)}`,price:item.product.price,currency:item.product.currency}}));await db.send(new PutCommand({TableName:table,Item:{id:postId,ownerId:id,sourceOutfitId:outfitId,sourceType:"consumer",handle:`@synthetic_demo_${suffix}`,outfitTitle:`Synthetic Consumer Look ${suffix}`,caption:"Clearly labeled synthetic Community outfit.",image:"",createdAt:outfitCreated,likes:index%8,publishedGarments,garments:[],products:[],fictional:true,testCohort:true,dataClassification:"DEMO",PK:"COMMUNITY",SK:`POST#${outfitCreated}#${postId}`}}));for(let eventIndex=0;eventIndex<index%4;eventIndex++){const eventAt=new Date(Date.parse(outfitCreated)+eventIndex*1000).toISOString();await db.send(new PutCommand({TableName:table,Item:{PK:"COMMUNITY",SK:`EVENT#${eventAt}#demo-recreate-${suffix}-${eventIndex}`,postId,eventType:"recreate-look-request",createdAt:eventAt,dataClassification:"DEMO"}}));}}
  consumers.push(email);
}

const recreateId="demo-consumer-recreate",recreateEmail="demo.consumer.recreate@racked.local";
await db.send(new PutCommand({TableName:table,Item:await accountItem({id:recreateId,email:recreateEmail,role:"consumer",displayName:"Synthetic Recreate Consumer",brandDataSharing:false})}));
for(const [cohortKey,productIndex,garmentId] of [["atelier",0,"demo-recreate-exact-top"],["stride",1,"demo-recreate-similar-shoe"]]){const cohort=cohorts.find(item=>item.key===cohortKey),product=productSets.get(cohortKey)[productIndex],imageKey=`wardrobe/${recreateId}/${garmentId}.png`,source=await imageFor(cohort,productIndex,"front");await s3.send(new PutObjectCommand({Bucket:bucket,Key:imageKey,Body:source,ContentType:"image/png",ServerSideEncryption:"AES256",Metadata:{owner:recreateId,synthetic:"true",classification:"demo"}}));await db.send(new PutCommand({TableName:table,Item:{id:garmentId,name:product.name,category:product.category,subtype:product.category==="shoe"?"sneakers":"t-shirt",color:cohort.base,pattern:"solid",material:"synthetic demo material",style:["casual","synthetic demo"],season:"all-season",wearCount:0,lastWornDays:999,source:"ai-confirmed",art:"photo",imageKey,brand:product.brand,sku:product.sku,registryProductId:product.id,identityStatus:"verified",createdAt,testCohort:true,dataClassification:"DEMO",PK:`USER#${recreateId}`,SK:`GARMENT#${garmentId}`,GSI1PK:`PRODUCT#${product.id}`,GSI1SK:`OWNER#${recreateId}`}}));}

console.log(JSON.stringify({brands:cohorts.map(cohort=>({email:cohort.email,brand:cohort.brand,products:10,looks:2})),consumerAccounts:[...consumers,recreateEmail],recreateDemo:{email:recreateEmail,publicPostId:"demo-consumer-post-01",expected:{exact:"RTA-001",similar:"SSL-002",missingCategory:"jewelry"}},passwordSource:"RACKED_TEST_PASSWORD",cohortSize:consumers.length,wearEventsPerHeroProduct:consumers.reduce((sum,_,index)=>sum+wearPattern[index%wearPattern.length],0),clearlyLabeledSyntheticData:true,dataClassification:"DEMO"},null,2));
