...
import { demoProductImagePath } from "./demo-storefront.ts";
...
function publicGarment(postId:string, stored:StoredPublishedGarment, demoPost:boolean):PublicOutfitGarment {
  ...
  return {
    ...
    image:(demoPost&&verified?demoProductImagePath(verified.sku):undefined)??(stored.imageKey?imagePath(postId,String(stored.publicGarmentId)):""),
...
export function toPublicOutfitPost(stored: StoredCommunityPost): OutfitPost {
  ...
  const demoPost=stored.dataClassification==="DEMO"||stored.fictional===true;
  const garments=(Array.isArray(stored.publishedGarments)?stored.publishedGarments:[]).map((garment)=>publicGarment(id,garment,demoPost));
