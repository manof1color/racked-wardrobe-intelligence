...
Every merged PR above ran `pnpm lint`, `pnpm test`, `pnpm build`, and a production dependency audit in CI (`pnpm typecheck` joined the pipeline at #27), passed CodeQL, and was deployed by AWS Amplify from `main` with the live URL spot-checked after each merge. The repository now carries **192 passing tests** (re-verified 2026-08-21). PR #75 passed Validate Racked and CodeQL, merged as `d8fca8f`, and its Amplify replacement was confirmed through the public `racked-shell-v2` artifact timestamped 2026-08-21 23:20 UTC. The public Consumer route loads and preserves the secure sign-in boundary; the account-specific Hanger revision and piece-removal clicks still require a physical-phone session retest. SES-backed reset delivery, a live production-table match run, and a Brand-session return-path check remain explicit environment follow-ups.
