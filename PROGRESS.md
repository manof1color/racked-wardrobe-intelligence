...
Every merged PR above ran `pnpm lint`, `pnpm test`, `pnpm build`, and a production dependency audit in CI (`pnpm typecheck` joined the pipeline at #27), passed CodeQL, and was deployed by AWS Amplify from `main` with the live URL spot-checked after each merge. The current `main` branch carries **259 passing tests** (re-verified 2026-08-29). Pair-aware detection and AI cutout quality still require a physical-phone production retest on a crowded real shoe rack; automated checks validate contracts and output handling but do not constitute measured recognition or segmentation accuracy. SES-backed reset delivery, a live production-table match run, and a Brand-session return-path check remain explicit environment follow-ups.
