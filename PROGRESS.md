...
Every merged PR above ran `pnpm lint`, `pnpm test`, `pnpm build`, and a production dependency audit in CI (`pnpm typecheck` joined the pipeline at #27), passed CodeQL, and was deployed by AWS Amplify from `main` with the live URL spot-checked after each merge. The repository now carries **214 passing tests** (re-verified 2026-08-22). Pair-aware detection still requires a physical-phone production retest on a crowded real shoe rack after deployment; automated checks validate the contract and parser behavior but do not constitute a measured recognition-recall claim. SES-backed reset delivery, a live production-table match run, and a Brand-session return-path check remain explicit environment follow-ups.
