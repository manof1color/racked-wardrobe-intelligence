...
- [#80](https://github.com/manof1color/racked-wardrobe-intelligence/pull/80) separates explicit garment instructions from preference scoring. Exact saved names, unique garment shorthand, and unique color-plus-subtype descriptions resolve only against the signed-in wardrobe and are locked before occasion, compatibility, or underuse scoring. Current “keep” requests override prior-suggestion rotation; exclusions, ambiguity, and unknown garments fail safely instead of forcing the wrong piece. Hanger visibly acknowledges required pieces instead of claiming they were selected for low wear.

## Continuous verification

Every merged PR above ran `pnpm lint`, `pnpm test`, `pnpm build`, and a production dependency audit in CI (`pnpm typecheck` joined the pipeline at #27), passed CodeQL, and was deployed by AWS Amplify from `main` with the live URL spot-checked after each merge. The repository now carries **206 passing tests** (re-verified 2026-08-22). PR #78 passed Validate Racked and CodeQL, merged as `50e1e0d`, and its Amplify replacement was confirmed through the public artifact timestamped 2026-08-22 04:28 UTC. The landing and Consumer routes both returned HTTP 200, and the Consumer route preserved the secure sign-in boundary. Account-specific required-piece interpretation, canonical selection saving, and piece-removal clicks still require a physical-phone session retest after the latest deployment. SES-backed reset delivery, a live production-table match run, and a Brand-session return-path check remain explicit environment follow-ups.
