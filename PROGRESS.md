...
Every merged PR above ran `pnpm lint`, `pnpm test`, `pnpm build`, and a production dependency audit in CI (`pnpm typecheck` joined the pipeline at #27), passed CodeQL, and was deployed by AWS Amplify from `main` with the live URL spot-checked after each merge. Through #61, `main` carries **151 passing tests** (re-verified 2026-08-16). The authenticated Consumer route, public-route session redirects, Add deep link, and mobile Community navigation were confirmed live after #59; the new compact menu's production visual check, a live Brand-session return-path check, and a physical-iPhone whole-look/cutout retest remain explicit device/account follow-ups.
