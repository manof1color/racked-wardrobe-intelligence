...
- Consumer Hanger now recognizes natural revision wording such as “adjust,” “redo,” “remake,” “try again,” and “use my other pieces,” then renders the exact selected owned garment photos before Save. The client reliably finds the most recent save action even when intervening replies have no actions. Saved outfits support owner-scoped, two-step removal of individual pieces; the server revalidates every remaining wardrobe ID and regenerates the private flat-lay at a new cache-distinct object key without deleting the wardrobe garment. Installed home-screen clients actively check for service-worker updates so a deployed fix replaces the running app shell.

## Continuous verification

Every merged PR above ran `pnpm lint`, `pnpm test`, `pnpm build`, and a production dependency audit in CI (`pnpm typecheck` joined the pipeline at #27), passed CodeQL, and was deployed by AWS Amplify from `main` with the live URL spot-checked after each merge. The repository now carries **192 passing tests** (re-verified 2026-08-21). The authenticated Consumer route, public-route session redirects, Add deep link, mobile Community navigation, compact Consumer menu, explicit camera/library controls, fictional product imagery, $0.00 commerce simulation, and prior post-merge Amplify build replacements have been confirmed live. The Phase 15 branch remains subject to CI, CodeQL, Amplify replacement, and a physical-phone Hanger revision retest before its new behavior is claimed live. SES-backed reset delivery, a live production-table match run, and a Brand-session return-path check remain explicit environment follow-ups.
