...
export async function POST(request:Request) {
  ...
  try {const account=await authenticateAccount(body.email,body.password);if(!account)return NextResponse.json({error:"Email or password was incorrect."},{status:401});const secret=process.env.SESSION_SECRET;if(!secret)return NextResponse.json({error:"Session security is not configured."},{status:503});const token=await createSessionToken({subject:account.id,role:account.role,sessionVersion:account.sessionVersion??0,expiresAt:Date.now()+1000*60*60*24*7},secret);const response=NextResponse.json({destination:account.role==="consumer"?"/consumer":"/brand"});response.cookies.set(SESSION_COOKIE,token,{httpOnly:true,sameSite:"lax",secure:process.env.NODE_ENV==="production",path:"/",maxAge:60*60*24*7});return response;}catch(error){
