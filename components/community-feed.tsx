...
export function CommunityFeed({initialPosts,canPost,savedOutfits}:{initialPosts:OutfitPost[];canPost:boolean;savedOutfits:SavedOutfitOption[]}) {
  ...
  return <>
    ...
    {posts.length===0?<div className="empty-wardrobe"><h2>No public outfits yet.</h2><p>Be the first to share a look built from your saved wardrobe.</p></div>:<div className="community-grid">{posts.map(post=>{
      ...
      return <article className="social-card" key={post.id}><div className="social-image">{post.image?<img src={post.image} alt={post.outfitTitle}/>:<div className="social-image-empty">Image unavailable</div>}</div><div className="social-copy"><div className="social-author"><span>{post.handle}</span><span>{post.sourceType==="brand"?"BRAND LOOK":"CONSUMER LOOK"}</span></div><h2>{post.outfitTitle}</h2><p>{post.caption}</p><div className="social-products">{post.garments.map(garment=>garment.verifiedProduct?<Link key={garment.publicGarmentId} href={`/brands/${garment.verifiedProduct.brandSlug}`}><span>{garment.verifiedProduct.brand}</span><strong>{garment.name}</strong><small>{garment.verifiedProduct.sku} · Verified product →</small></Link>:<div key={garment.publicGarmentId}><span>UNVERIFIED GARMENT</span><strong>{garment.name}</strong><small>{garment.subtype??garment.category}{garment.unverifiedBrandLabel?` · label: ${garment.unverifiedBrandLabel}`:""}</small></div>)}</div>
