...
export function ConsumerDashboard() {
  ...
  </main><nav className="mobile-tab-bar" aria-label="Mobile consumer navigation">{tabs.map((tab)=><button key={tab.id} className={activeView===tab.id?"active":""} onClick={()=>setActiveView(tab.id)}><span>{tab.icon}</span><small>{tab.label}</small></button>)}<button onClick={()=>setShowAdd(true)}><span>＋</span><small>Scan</small></button></nav>{showAdd&&<div className="modal-backdrop" role="presentation"><section className="modal upload-modal" role="dialog" aria-modal="true" aria-labelledby="add-title"><button className="modal-close" aria-label="Close" onClick={()=>setShowAdd(false)}>×</button><div className="eyebrow">THREE-VIEW GARMENT INTAKE</div><h2 id="add-title">Front. Back. Label.</h2><p>Images are validated and hashed by the backend. Label text, SKU/GTIN, and exact registered images can connect the item to a brand-enrolled product.</p><ThreeViewUploader onConfirmed={confirmAnalysis}/></section></div>}</AppShell>;
}
