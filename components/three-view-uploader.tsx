...
import { GARMENT_TAXONOMY, normalizeGarmentCategory, subtypeForCategory, type GarmentCategory } from "@/lib/garment-taxonomy";
...
export function ThreeViewUploader({onConfirmed}:{onConfirmed:(analysis:GarmentAnalysis,overrides:GarmentOverrides)=>Promise<void>}) {
  ...
  const [overrides,setOverrides]=useState<GarmentOverrides>({name:"",brand:"",sku:"",category:"unknown",subtype:"other-garment"});
  ...
  return <div className="three-view-flow">
    ...
    <div className="three-view-grid">{allViews.map(view=>{const request=slotRequest(view);return <article className={`view-upload ${files[view]?"has-file":""}`} key={view}>{previews[view]?<img src={previews[view]} alt={`${view} upload preview`}/>:<span className="view-placeholder">{view==="front"?"01":view==="back"?"02":"03"}</span>}<strong>{request?.title??view}</strong><small>{files[view]?.name??request?.instruction??"Add this view"}</small><PhotoSourcePicker compact label={`Add ${view} photo`} accept="image/jpeg,image/png,image/webp,image/heic,image/heif" onFile={file=>setViewFile(view,file)}/></article>;})}</div>
    ...
    {analysis&&<section className="analysis-result" aria-live="polite"><div className="analysis-head"><div><span className="fallback-pill">{analysis.label.matched?"VERIFIED PRODUCT":analysis.label.suggested?"SUGGESTED BRAND":"CUSTOM LABEL"}</span><h3>{analysis.garment.name}</h3><p>{analysis.garment.color} · {analysis.garment.subtype} · {analysis.garment.material}</p></div><div className="analysis-confidence"><strong>{analysis.confidence}%</strong><small>AI confidence</small></div></div>
      ...
      {analysis.garment.alternatives.length>0&&<div className="ai-notice"><span>OTHER POSSIBILITIES</span>{analysis.garment.alternatives.map(option=>`${option.subtype} ${option.confidence}%`).join(" · ")}</div>}
      <div className="consumer-label-fields"><label>Your garment label<input value={overrides.name} maxLength={100} onChange={event=>setOverrides({...overrides,name:event.target.value})}/><small>This is the name shown in your closet.</small></label><label>Broad category<select value={overrides.category} onChange={event=>setCorrectionCategory(event.target.value)}>{PLANNED_CATEGORIES.map(category=><option key={category} value={category}>{category}</option>)}</select><small>Used for outfit grouping.</small></label><label>Specific garment type<select value={overrides.subtype} onChange={event=>setOverrides({...overrides,subtype:event.target.value})}>{GARMENT_TAXONOMY[overrides.category].map(subtype=><option key={subtype} value={subtype}>{subtype}</option>)}</select><small>Confirm or correct the AI subtype.</small></label><label>Brand name<input value={overrides.brand} maxLength={100} onChange={event=>setOverrides({...overrides,brand:event.target.value})} placeholder="Confirm, edit, or add a brand"/><small>{analysis.label.matched?"Verified from the enrolled brand registry.":"Your label will remain unverified unless it matches an enrolled product."}</small></label><label>SKU / style code <small>optional</small><input value={overrides.sku} maxLength={64} onChange={event=>setOverrides({...overrides,sku:event.target.value})}/></label></div>
